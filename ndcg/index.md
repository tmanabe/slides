...
とは、検索システムがユーザの役に立った度合いを測ること<sub>だと思います</sub>
...
- クエリに対するドキュメントランキングの評価尺度
    - Normalized: クエリごとに `[0, 1]` に正規化する
    - Discounted: 上位にランキングされたドキュメントほど重視する
...
## そもそも適合度に矛盾が生じる (continued)

### 対策

- nDCG をクエリごとではなく、検索結果の提示ごとに計算する
...
$$ave. nDCG = \frac{0.964 + 0.920 + 0.500}{3} = 0.795$$
...
A. そうとは言えない場合がありそう。例えばEコマースで：
- 頻繁に購買があるクエリの検索結果は、重点的に最適化したい
- そもそも適合な商品がないクエリの検索結果は、
