...
- 以上を考慮しつつ、短いドキュメントほどスコアが高い（正規化）
単に長いドキュメントにもキーワードが多く出現するのを補正
...
- とはいえ、どこまでも高くしてはいけない（たとえばスパム対策のため）
...
- 単に長いドキュメントにもキーワードが多く出現するのを補正するアイデア
- この補正の強さをパラメータ $b$ で制御（例では $0.75$）
...
- 実サービスだと、検索エンジンを一から作ることは稀<sub>（だと思います）</sub>なので、
...
- 実際、自然に両立できた（評価は未公開ですが……）<sup>https://arxiv.org/abs/1709.03260</sup>
